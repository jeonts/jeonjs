...
function visitArray(jeon, visit, jsonImpl, isTopLevel = false) {
...
function visitFunctionDeclaration$1(keys, jeon, visit, jsonImpl) {
...
function visitVariableDeclaration$1(op, operands, visit, jsonImpl) {
  const declarationType = op === "@@" ? "const" : "let";
  if (operands["="] && Array.isArray(operands["="])) {
    const [pattern, source2] = operands["="];
    const patternStr = convertDestructuringPattern(pattern);
    const sourceStr = visit(source2);
...
function convertDestructuringPattern(pattern) {
  const elements = [];
...
function visitArrowFunction(op, operands, visit, jsonImpl) {
...
function visitJSX(keys, jeon, visit, jsonImpl, useJsxFunction = false) {
  ...
  if (componentKey) {
    ...
    if (useJsxFunction) {
      ...
      const propStrings = [];
      for (const [key2, value] of Object.entries(processedProps)) {
        if (key2 === "children") {
          if (typeof value === "string" && value.includes("jsx(")) {
            propStrings.push(`${key2}: ${value}`);
          } else if (Array.isArray(value)) {
            const childrenArrayItems = value.map((child) => {
              if (typeof child === "string" && child.includes("jsx(")) {
                return child;
              }
              return jsonImpl ? jsonImpl.stringify(child) : JSON.stringify(child);
            });
            propStrings.push(`${key2}: [${childrenArrayItems.join(", ")}]`);
          } else {
            propStrings.push(`${key2}: ${jsonImpl ? jsonImpl.stringify(value) : JSON.stringify(value)}`);
          }
        } else {
          propStrings.push(`${key2}: ${jsonImpl ? jsonImpl.stringify(value) : JSON.stringify(value)}`);
        }
...
function visitClass(op, operands, keys, visit, jsonImpl) {
...
function visitTryCatch(operands, visit, jsonImpl) {
  if (operands) {
    return generateTryCatchBlock(operands, visit);
  }
  return "";
}
function visitOperator(op, operands, visit, jsonImpl) {
  switch (op) {
    case "-":
    case "+":
    case "!":
...
function visitFunctionCall(keys, jeon, visit, jsonImpl) {
  return "";
}
function visitObject(keys, jeon, visit, jsonImpl) {
...
function visitPropertyAccess(op, operands, visit, jsonImpl) {
  if (op === ".") {
    if (Array.isArray(operands) && operands.length >= 2) {
...
function visitSequencing(op, operands, visit, jsonImpl) {
...
function visitSwitch(operands, visit, jsonImpl) {
  if (!Array.isArray(operands) || operands.length !== 2) {
    return "";
  }
...
function jeon2js(jeon, options2, isTopLevel = true) {
  ...
  const useJsxFunction = options2?.jsx || false;
  const visit = (item) => jeon2js(item, { json: jsonImpl, jsx: useJsxFunction }, false);
  ...
  if (Array.isArray(jeon)) {
    return visitorRegistry$1.visitArray(jeon, visit, jsonImpl, isTopLevel);
  }
  if (typeof jeon === "object" && jeon !== null) {
    ...
    const functionDeclarationResult = visitorRegistry$1.visitFunctionDeclaration(keys, jeon, visit, jsonImpl);
    ...
    if (keys.length === 1) {
      ...
      const operatorResult = visitorRegistry$1.visitOperator(op, operands, visit, jsonImpl);
      if (operatorResult) {
        return operatorResult;
      }
      if (op === "try") {
        return visitorRegistry$1.visitTryCatch(operands, visit, jsonImpl);
      }
      if (op === "@" || op === "@@") {
        return visitorRegistry$1.visitVariableDeclaration(op, operands, visit, jsonImpl);
      }
      const arrowFunctionResult = visitorRegistry$1.visitArrowFunction(op, operands, visit, jsonImpl);
      if (arrowFunctionResult) {
        return arrowFunctionResult;
      }
      const classResult = visitorRegistry$1.visitClass(op, operands, keys, visit, jsonImpl);
      if (classResult) {
        return classResult;
      }
      const sequencingResult = visitorRegistry$1.visitSequencing(op, operands, visit, jsonImpl);
      if (sequencingResult) {
        return sequencingResult;
      }
      const propertyAccessResult = visitorRegistry$1.visitPropertyAccess(op, operands, visit, jsonImpl);
      if (propertyAccessResult) {
        return propertyAccessResult;
      }
      if (op === "switch") {
        return visitorRegistry$1.visitSwitch(operands, visit, jsonImpl);
      }
    }
    const jsxResult = visitorRegistry$1.visitJSX(keys, jeon, visit, jsonImpl, useJsxFunction);
    if (jsxResult) {
      return jsxResult;
    }
    return visitorRegistry$1.visitObject(keys, jeon, visit, jsonImpl);
  }
...
function visitCallExpression(node, options2) {
  if (node.callee.type === "MemberExpression") {
    ...
    if ((calleeNode.type === "FunctionExpression" || calleeNode.type === "ArrowFunctionExpression") && node.arguments.length > 0) {
      return {
        "()": [
          ast2jeon(calleeNode, options2),
          ...node.arguments.map((arg) => ast2jeon(arg, options2))
        ]
      };
...
function visitMemberExpression(node, options2) {
  if (node.computed) {
    return {
      "[]": [
        ast2jeon(node.object, options2),
        ast2jeon(node.property, options2)
      ]
    };
  }
  const segments = [];
  let current2 = node;
  while (current2 && current2.type === "MemberExpression" && !current2.computed) {
    if (current2.property.type === "Identifier") {
      segments.unshift(current2.property.name);
    } else {
...
function visitArrayExpression(node, options2) {
  return node.elements.map((element) => {
    ...
    return element ? ast2jeon(element, options2) : null;
  });
}
...
function visitNewExpression(node, options2) {
  ...
  if (node.callee.type === "ParenthesizedExpression") {
    calleeExpression = node.callee.expression;
  } else {
    calleeExpression = node.callee;
  }
  return {
    "new": [
      ast2jeon(calleeExpression, options2),
      ...node.arguments.map((arg) => ast2jeon(arg, options2))
    ]
  };
}
...
function visitArrowFunctionExpression(node, options2) {
  ...
  return {
    [`${paramString} =>`]: ast2jeon(node.body, options2)
  };
}
...
function visitVariableDeclaration(node, options2) {
  ...
  for (const decl of node.declarations) {
    if (decl.id.type === "Identifier") {
      ...
      const initExpr = decl.init ? ast2jeon(decl.init, options2) : null;
      const pattern = [];
      ...
      for (let i = 0; i < arrayPattern.elements.length; i++) {
        ...
        if (element) {
          if (element.type === "Identifier") {
            ...
            const nestedPattern = convertNestedArrayPattern(element);
...
function convertNestedArrayPattern(pattern) {
  const elements = [];
  for (const element of pattern.elements) {
    if (!element) {
      ...
      elements.push(convertNestedArrayPattern(element));
    } else if (element.type === "ObjectPattern") {
...
function visitArrayPattern(node, options2) {
  ...
  for (let i = 0; i < node.elements.length; i++) {
    ...
    if (element) {
      if (element.type === "Identifier") {
        elements[i.toString()] = `@${element.name}`;
      } else if (element.type === "RestElement") {
        const arg = element.argument;
        if (arg && arg.type === "Identifier") {
          elements[`...${i}`] = `@${arg.name}`;
        ...
        elements[`[${i}]`] = visitArrayPattern(element, options2);
      } else if (element.type === "ObjectPattern") {
        elements[`{${i}}`] = `[ObjectPattern]`;
      } else {
        elements[i.toString()] = ast2jeon(element, options2);
      }
    } else {
...
function visitSequenceExpression(node, options2) {
  return {
    ",": node.expressions.map((expr) => ast2jeon(expr, options2))
  };
}
function visitIIFE(node, options2) {
  if (!options2?.iife) {
    return ast2jeon(node, options2);
  }
  switch (node.type) {
    case "Program":
      return handleProgramIIFE(node, options2);
    case "BlockStatement":
      return handleBlockIIFE(node, options2);
    case "ExpressionStatement":
      return handleExpressionIIFE(node.expression, options2);
    default:
      return ast2jeon(node, options2);
  }
}
function handleProgramIIFE(node, options2) {
  if (node.body.length === 1) {
    ...
    if (firstStatement.type === "ExpressionStatement") {
      ...
      switch (expression.type) {
        ...
        default:
          return ast2jeon(firstStatement, options2);
      }
    } else if (firstStatement.type === "BlockStatement") {
      return handleBlockIIFE(firstStatement, options2);
    } else {
      return ast2jeon(firstStatement, options2);
    }
  } else if (node.body.length > 1) {
    return handleMultipleStatementsIIFE(node.body, options2);
  }
  return ast2jeon(node, options2);
}
function handleBlockIIFE(node, options2) {
    ...
    if (lastStatement.type === "ReturnStatement") {
      return {
        "(": {
          "()": [
            {
              "() =>": ast2jeon(node, options2)
              // Body with no parameters
            },
            []
            // No arguments
          ]
...
function handleExpressionIIFE(node, options2) {
...
function handleMultipleStatementsIIFE(statements, options2) {
      ...
      if (lastStatement.type === "ReturnStatement") {
        ...
        return {
          "(": {
            "()": [
              {
                "() =>": ast2jeon(blockStatement, options2)
                // Body with no parameters
              },
              []
              // No arguments
  ...
  return statements.map((stmt) => ast2jeon(stmt, options2));
}
const visitorRegistry = {
  ...
  "Program": (node, options2) => {
    if (options2 && options2.iife) {
      const visitor = visitorRegistry["IIFE"];
      if (visitor) {
        return visitor(node, options2);
      }
    }
    return visitProgram(node, options2);
  },
  ...
  "ArrayPattern": (node, options2) => visitArrayPattern(node, options2),
  ...
  "SequenceExpression": (node, options2) => visitSequenceExpression(node, options2),
  "IIFE": (node, options2) => visitIIFE(node, options2)
};
  ...
  switch (node.type) {
    ...
    case "ArrowFunctionExpression":
      ...
      if (node.type === "ArrowFunctionExpression") {
        const paramStr = params.length > 0 ? `(${params.join(", ")})` : "()";
        return {
          [`${paramStr} =>`]: ast2jeon(funcNode.body, options2)
        };
      } else {
      ...
      for (const decl of node.declarations) {
        if (decl.id.type === "Identifier") {
          ...
          for (const prop of decl.id.properties) {
            if (prop.type === "Property" && prop.key.type === "Identifier") {
              const propName = prop.key.name;
              declarations[propName] = `@[${propName}]`;
            }
          }
        } else if (decl.id.type === "ArrayPattern") {
          const initExpr = decl.init ? ast2jeon(decl.init, options2) : null;
    ...
    case "Literal":
      ...
      let processedValue = literalValue;
      ...
      const cases = switchStmt.cases.map((caseClause) => {
        if (caseClause.test) {
          return {
            "case": ast2jeon(caseClause.test, options2),
            "body": {
              "statements": caseClause.consequent.map((consequent) => ast2jeon(consequent, options2))
            }
          };
        } else {
          return {
...
function js2jeon(code, options2) {
  try {
    ...
    if (options2?.iife) {
      ...
      try {
        const Parser22 = Parser.extend(jsx2());
        ast2 = Parser22.parse(trimmedCode, {
          ecmaVersion: "latest",
          sourceType: "module",
          allowReturnOutsideFunction: true,
          preserveParens: true
        });
        if (ast2.body.length === 1 && ast2.body[0].type === "BlockStatement" && ast2.body[0].body.length === 0) {
          shouldTryParentheses = true;
        } else if (ast2.body.length === 1 && ast2.body[0].type === "ExpressionStatement" && ast2.body[0].expression.type === "ObjectExpression") {
        }
        parseSuccess = !shouldTryParentheses;
...
function evaluatePlus(operands, context2) {
  if (Array.isArray(operands)) {
...
function evaluateMinus(operands, context2) {
  if (Array.isArray(operands)) {
    if (operands.length === 1) {
      return -evalJeon(operands[0], context2);
    } else if (operands.length >= 2) {
...
function evaluateMultiply(operands, context2) {
  if (Array.isArray(operands) && operands.length >= 2) {
    return operands.reduce(
      (acc, curr) => evalJeon(acc, context2) * evalJeon(curr, context2)
    );
...
function evaluateDivide(operands, context2) {
  if (Array.isArray(operands) && operands.length >= 2) {
    return operands.slice(1).reduce(
      (acc, curr) => acc / evalJeon(curr, context2),
      evalJeon(operands[0], context2)
...
function evaluateModulo(operands, context2) {
  if (Array.isArray(operands) && operands.length >= 2) {
    return operands.slice(1).reduce(
      (acc, curr) => acc % evalJeon(curr, context2),
      evalJeon(operands[0], context2)
...
function evaluateEqual(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    return evalJeon(operands[0], context2) == evalJeon(operands[1], context2);
  }
}
function evaluateStrictEqual(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    return evalJeon(operands[0], context2) === evalJeon(operands[1], context2);
  }
}
function evaluateNotEqual(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    return evalJeon(operands[0], context2) != evalJeon(operands[1], context2);
  }
}
function evaluateStrictNotEqual(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    return evalJeon(operands[0], context2) !== evalJeon(operands[1], context2);
  }
}
function evaluateLessThan(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    return evalJeon(operands[0], context2) < evalJeon(operands[1], context2);
  }
}
function evaluateGreaterThan(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    return evalJeon(operands[0], context2) > evalJeon(operands[1], context2);
  }
}
function evaluateLessThanOrEqual(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    return evalJeon(operands[0], context2) <= evalJeon(operands[1], context2);
  }
}
function evaluateGreaterThanOrEqual(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    return evalJeon(operands[0], context2) >= evalJeon(operands[1], context2);
  }
}
function evaluateLogicalAnd(operands, context2) {
  if (Array.isArray(operands) && operands.length >= 2) {
    return operands.every((operand) => evalJeon(operand, context2));
  }
}
function evaluateLogicalOr(operands, context2) {
  if (Array.isArray(operands) && operands.length >= 2) {
    return operands.some((operand) => evalJeon(operand, context2));
  }
}
function evaluateLogicalNot(operands, context2) {
  if (operands !== void 0) {
    return !evalJeon(operands, context2);
  }
}
function evaluateBitwiseNot(operands, context2) {
  if (operands !== void 0) {
    return ~evalJeon(operands, context2);
  }
}
function evaluateBitwiseAnd(operands, context2) {
  if (Array.isArray(operands) && operands.length >= 2) {
    return operands.reduce(
      (acc, curr) => evalJeon(acc, context2) & evalJeon(curr, context2)
    );
...
function evaluateBitwiseOr(operands, context2) {
  if (Array.isArray(operands) && operands.length >= 2) {
    return operands.reduce(
      (acc, curr) => evalJeon(acc, context2) | evalJeon(curr, context2)
    );
...
function evaluateBitwiseXor(operands, context2) {
  if (Array.isArray(operands) && operands.length >= 2) {
    return operands.reduce(
      (acc, curr) => evalJeon(acc, context2) ^ evalJeon(curr, context2)
    );
...
function evaluateLeftShift(operands, context2) {
  if (Array.isArray(operands) && operands.length >= 2) {
    return operands.slice(1).reduce(
      (acc, curr) => acc << evalJeon(curr, context2),
      evalJeon(operands[0], context2)
...
function evaluateRightShift(operands, context2) {
  if (Array.isArray(operands) && operands.length >= 2) {
    return operands.slice(1).reduce(
      (acc, curr) => acc >> evalJeon(curr, context2),
      evalJeon(operands[0], context2)
...
function evaluateUnsignedRightShift(operands, context2) {
  if (Array.isArray(operands) && operands.length >= 2) {
    return operands.slice(1).reduce(
      (acc, curr) => acc >>> evalJeon(curr, context2),
      evalJeon(operands[0], context2)
...
function evaluateTypeof(operands, context2) {
  if (operands !== void 0) {
    return typeof evalJeon(operands, context2);
  }
}
function evaluateVoid(operands, context2) {
  if (operands !== void 0) {
    evalJeon(operands, context2);
    return void 0;
  }
}
function evaluateDelete(operands, context2) {
  return true;
}
function evaluateAssignment(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    ...
    if (varNameExpr && typeof varNameExpr === "object" && !Array.isArray(varNameExpr) && "." in varNameExpr) {
      const propAccessExpr = varNameExpr["."];
      if (Array.isArray(propAccessExpr) && propAccessExpr.length >= 2) {
        const obj = evalJeon(propAccessExpr[0], context2);
...
function evaluateAddAssignment(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    const varNameExpr = operands[0];
    const valueToAdd = evalJeon(operands[1], context2);
    let varName;
    if (typeof varNameExpr === "string" && varNameExpr.startsWith("@")) {
      varName = varNameExpr.substring(1);
    } else if (typeof varNameExpr === "string") {
    ...
    const currentValue = context2[varName] ?? 0;
    const newValue = currentValue + valueToAdd;
    context2[varName] = newValue;
    return newValue;
  }
}
function evaluateSubtractAssignment(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    const varNameExpr = operands[0];
    const valueToSubtract = evalJeon(operands[1], context2);
    let varName;
    if (typeof varNameExpr === "string" && varNameExpr.startsWith("@")) {
      varName = varNameExpr.substring(1);
    } else if (typeof varNameExpr === "string") {
...
function evaluateMultiplyAssignment(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    const varNameExpr = operands[0];
    const valueToMultiply = evalJeon(operands[1], context2);
    let varName;
    if (typeof varNameExpr === "string" && varNameExpr.startsWith("@")) {
      varName = varNameExpr.substring(1);
    } else if (typeof varNameExpr === "string") {
...
function evaluateDivideAssignment(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    const varNameExpr = operands[0];
    const valueToDivide = evalJeon(operands[1], context2);
    let varName;
    if (typeof varNameExpr === "string" && varNameExpr.startsWith("@")) {
      varName = varNameExpr.substring(1);
    } else if (typeof varNameExpr === "string") {
...
function evaluateModuloAssignment(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    const varNameExpr = operands[0];
    const valueToModulo = evalJeon(operands[1], context2);
    let varName;
    if (typeof varNameExpr === "string" && varNameExpr.startsWith("@")) {
      varName = varNameExpr.substring(1);
    } else if (typeof varNameExpr === "string") {
...
function evaluateLeftShiftAssignment(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    const varNameExpr = operands[0];
    const valueToShift = evalJeon(operands[1], context2);
    let varName;
    if (typeof varNameExpr === "string" && varNameExpr.startsWith("@")) {
      varName = varNameExpr.substring(1);
    } else if (typeof varNameExpr === "string") {
...
function evaluateRightShiftAssignment(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    const varNameExpr = operands[0];
    const valueToShift = evalJeon(operands[1], context2);
    let varName;
...
function evaluateUnsignedRightShiftAssignment(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    const varNameExpr = operands[0];
    const valueToShift = evalJeon(operands[1], context2);
    let varName;
...
function evaluateBitwiseAndAssignment(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    const varNameExpr = operands[0];
    const valueToAnd = evalJeon(operands[1], context2);
    let varName;
    if (typeof varNameExpr === "string" && varNameExpr.startsWith("@")) {
      varName = varNameExpr.substring(1);
    } else if (typeof varNameExpr === "string") {
...
function evaluateBitwiseXorAssignment(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    const varNameExpr = operands[0];
    const valueToXor = evalJeon(operands[1], context2);
    let varName;
    if (typeof varNameExpr === "string" && varNameExpr.startsWith("@")) {
      varName = varNameExpr.substring(1);
    } else if (typeof varNameExpr === "string") {
...
function evaluateBitwiseOrAssignment(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    const varNameExpr = operands[0];
    const valueToOr = evalJeon(operands[1], context2);
    let varName;
    if (typeof varNameExpr === "string" && varNameExpr.startsWith("@")) {
      varName = varNameExpr.substring(1);
    } else if (typeof varNameExpr === "string") {
...
function evaluateIncrement(operands, context2) {
  if (operands !== void 0) {
...
function evaluateDecrement(operands, context2) {
  if (operands !== void 0) {
    const varNameExpr = operands;
    let varName;
    if (typeof varNameExpr === "string" && varNameExpr.startsWith("@")) {
...
function evaluateIf(operands, context2) {
...
function evaluateWhile(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    let result;
    while (evalJeon(operands[0], context2)) {
      result = evalJeon(operands[1], context2);
    }
    return result;
  }
}
function evaluateFor(operands, context2) {
  if (Array.isArray(operands) && operands.length === 4) {
    evalJeon(operands[0], context2);
    let result;
    while (evalJeon(operands[1], context2)) {
      result = evalJeon(operands[3], context2);
      evalJeon(operands[2], context2);
    }
    return result;
  }
}
function evaluateReturn(operands, context2) {
  return evalJeon(operands, context2);
}
function evaluateYield(operands, context2) {
  throw new Error("yield can only be used inside generator functions");
}
function evaluateYieldStar(operands, context2) {
  throw new Error("yield* can only be used inside generator functions");
}
function evaluateFunctionCall(operands, context2) {
  if (Array.isArray(operands) && operands.length >= 1) {
...
function evaluateFunction(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
...
function evaluateAsyncFunction(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    const [params, body] = operands;
...
function evaluateGeneratorFunction(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    const [params, body] = operands;
...
function evaluateArrowFunction(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    const [params, body] = operands;
    return function(...args) {
      const functionContext = { ...context2 };
...
function evaluatePropertyAccess(operands, context2) {
  if (Array.isArray(operands) && operands.length >= 2) {
    ...
    for (let i = 1; i < operands.length; i++) {
      ...
      if (typeof propOperand === "string") {
        if (propOperand.startsWith("@")) {
          ...
          prop = context2[cleanName] ?? void 0;
        } else {
          prop = propOperand;
        }
      } else {
        prop = evalJeon(propOperand, context2);
      }
      if (result && (typeof result === "object" || typeof result === "function") && prop in result) {
        result = result[prop];
        if (typeof result === "function" && i === operands.length - 1) {
          const isBuiltInStaticMethod = obj === Symbol && (prop === "for" || prop === "keyFor") || obj === Object && ["assign", "create", "defineProperties", "defineProperty", "entries", "freeze", "fromEntries", "getOwnPropertyDescriptor", "getOwnPropertyDescriptors", "getOwnPropertyNames", "getOwnPropertySymbols", "getPrototypeOf", "is", "isExtensible", "isFrozen", "isSealed", "keys", "preventExtensions", "seal", "setPrototypeOf", "values"].includes(prop) || obj === Array && ["from", "isArray", "of"].includes(prop) || obj === Math && ["abs", "acos", "acosh", "asin", "asinh", "atan", "atan2", "atanh", "cbrt", "ceil", "clz32", "cos", "cosh", "exp", "expm1", "floor", "fround", "hypot", "imul", "log", "log10", "log1p", "log2", "max", "min", "pow", "random", "round", "sign", "sin", "sinh", "sqrt", "tan", "tanh", "trunc"].includes(prop);
...
function evaluateBracketAccess(operands, context2) {
  if (Array.isArray(operands) && operands.length === 2) {
    const obj = evalJeon(operands[0], context2);
    ...
    if (typeof keyOperand === "string") {
      if (keyOperand.startsWith("@")) {
        ...
        key2 = context2[cleanName] ?? void 0;
      } else {
        key2 = keyOperand;
      }
    } else {
      key2 = evalJeon(keyOperand, context2);
    }
    if (obj && (typeof obj === "object" || typeof obj === "function") && key2 !== void 0) {
      return obj[key2];
    }
    return void 0;
  }
}
function evaluateSpread(operands, context2) {
  return evalJeon(operands, context2);
}
function evaluateParentheses(operands, context2) {
  return evalJeon(operands, context2);
}
function evaluateRegex(operands, context2) {
  if (typeof operands === "object" && operands !== null) {
...
function evaluateConditional(operands, context2) {
  if (Array.isArray(operands) && operands.length === 3) {
    const condition = evalJeon(operands[0], context2);
    return condition ? evalJeon(operands[1], context2) : evalJeon(operands[2], context2);
  }
}
function evaluateNew(operands, context2) {
  if (Array.isArray(operands) && operands.length >= 1) {
...
function evaluateSequence(operands, context2) {
  if (Array.isArray(operands)) {
    let result;
    for (const operand of operands) {
      result = evalJeon(operand, context2);
      if (operand && typeof operand === "object" && !Array.isArray(operand)) {
        const operandKeys = Object.keys(operand);
        for (const key2 of operandKeys) {
          if (key2.startsWith("function ") && key2.includes("(")) {
            const nameMatch = key2.match(/function\s+(\w+)\s*\(/);
...
function evaluateAwait(operands, context2) {
  throw new Error("await can only be used inside async functions");
}
function evaluateLineComment(operands, context2) {
  return void 0;
}
function evaluateBlockComment(operands, context2) {
  return void 0;
}
function evaluateClass(operands, context2) {
  if (typeof operands === "object" && operands !== null) {
    const ClassConstructor = function(...args) {
      const constructorKey = Object.keys(operands).find((key2) => key2.startsWith("constructor"));
      if (constructorKey) {
        const constructorBody = operands[constructorKey];
        const paramMatch = constructorKey.match(/\(([^)]*)\)/);
        const params = paramMatch ? paramMatch[1].split(",").map((p) => p.trim()).filter((p) => p) : [];
        const constructorContext = { this: this };
        params.forEach((param, index) => {
    ...
    const methodKeys = Object.keys(operands).filter((key2) => key2.includes("(") && !key2.startsWith("constructor"));
    methodKeys.forEach((methodKey) => {
      const isStatic = methodKey.startsWith("static ");
      const actualMethodKey = isStatic ? methodKey.substring(7) : methodKey;
      ...
      if (methodParamMatch && methodParamMatch[1]) {
        methodParams = methodParamMatch[1].split(",").map((p) => p.trim()).filter((p) => p);
      }
      if (isStatic) {
        ClassConstructor[methodName] = function(...args) {
          const methodContext = {};
          ...
          if (Array.isArray(methodBody)) {
            ...
            for (const statement of methodBody) {
              ...
              if (statement && typeof statement === "object" && !Array.isArray(statement) && statement["return"] !== void 0) {
                return result;
              }
            }
            return result;
...
function evalJeon(jeon, context2 = {}) {
  if (typeof jeon === "string") {
    ...
    if (context2[jeon] !== void 0) {
      return context2[jeon];
    }
    if (safeContext[jeon] !== void 0) {
  ...
  if (Array.isArray(jeon)) {
    ...
    if (!hasStatementObjects) {
      ...
      for (const item of jeon) {
        if (item === "@undefined") {
          ...
          result.push(evalJeon(item, context2));
        }
      }
      return result;
    }
    for (let i = 0; i < jeon.length; i++) {
      ...
      if (item && typeof item === "object" && !Array.isArray(item)) {
        ...
        const functionDeclarationKey = keys.find((key2) => key2.startsWith("function") || key2.startsWith("function*"));
        if (functionDeclarationKey) {
          ...
          if (nameMatch) {
            ...
            const params = paramStr ? paramStr.split(",").map((p) => p.trim()).filter((p) => p) : [];
            const body = item[functionDeclarationKey];
            const functionResult = function(...args) {
              console.log(`Calling function: ${functionName} with args:`, args);
              const functionContext = {};
              for (const key2 in context2) {
                functionContext[key2] = context2[key2];
              }
              params.forEach((param, index) => {
                functionContext[param] = args[index];
              });
            ...
            context2[functionName] = functionResult;
        ...
        const classDeclarationKey = keys.find((key2) => key2.startsWith("class "));
        if (classDeclarationKey) {
          const classNameMatch = classDeclarationKey.match(/class\s+(\w+)/);
          if (classNameMatch) {
            const className = classNameMatch[1];
            const classResult = evalJeon(item, context2);
            context2[className] = classResult;
            if (i === jeon.length - 1) {
              return void 0;
            }
            continue;
  ...
  if (typeof jeon === "object" && jeon !== null) {
    ...
    if (classDeclarationKey) {
      ...
      if (classNameMatch) {
        const className = classNameMatch[1];
        const classDef = jeon[classDeclarationKey];
        const ClassConstructor = function(...args) {
          const constructorKey = Object.keys(classDef).find((key2) => key2.startsWith("constructor"));
          if (constructorKey) {
        ...
        const methodKeys = Object.keys(classDef).filter((key2) => key2.includes("(") && !key2.startsWith("constructor"));
        methodKeys.forEach((methodKey) => {
          const isStatic = methodKey.startsWith("static ");
          const actualMethodKey = isStatic ? methodKey.substring(7) : methodKey;
          const methodName = actualMethodKey.split("(")[0];
          ...
          if (isStatic) {
            ClassConstructor[methodName] = function(...args) {
              ...
              if (Array.isArray(methodBody)) {
                let result2;
                for (const statement of methodBody) {
                  result2 = evalJeon(statement, methodContext);
                  if (statement && typeof statement === "object" && !Array.isArray(statement) && statement["return"] !== void 0) {
                    return result2;
                  }
                }
        ...
        context2[className] = ClassConstructor;
        return ClassConstructor;
      }
    }
    if (keys.length === 1) {
      ...
      switch (op) {
        case "-":
          return evaluateMinus(operands, context2);
        case "+":
          return evaluatePlus(operands, context2);
        case "*":
          return evaluateMultiply(operands, context2);
        case "/":
          return evaluateDivide(operands, context2);
        case "%":
          return evaluateModulo(operands, context2);
        case "==":
          return evaluateEqual(operands, context2);
        case "===":
          return evaluateStrictEqual(operands, context2);
        case "!=":
          return evaluateNotEqual(operands, context2);
        case "!==":
          return evaluateStrictNotEqual(operands, context2);
        case "<":
          return evaluateLessThan(operands, context2);
        case ">":
          return evaluateGreaterThan(operands, context2);
        case "<=":
          return evaluateLessThanOrEqual(operands, context2);
        case ">=":
          return evaluateGreaterThanOrEqual(operands, context2);
        case "&&":
          return evaluateLogicalAnd(operands, context2);
        case "||":
          return evaluateLogicalOr(operands, context2);
        case "!":
          return evaluateLogicalNot(operands, context2);
        case "~":
          return evaluateBitwiseNot(operands, context2);
        case "void":
          return evaluateVoid(operands, context2);
        case "delete":
          return evaluateDelete();
        case "&":
          return evaluateBitwiseAnd(operands, context2);
        case "|":
          return evaluateBitwiseOr(operands, context2);
        case "^":
          return evaluateBitwiseXor(operands, context2);
        case "<<":
          return evaluateLeftShift(operands, context2);
        case ">>":
          return evaluateRightShift(operands, context2);
        case ">>>":
          return evaluateUnsignedRightShift(operands, context2);
        case "typeof":
          return evaluateTypeof(operands, context2);
        case "/ /":
          return evaluateRegex(operands);
        case "(":
          return evaluateParentheses(operands, context2);
        case "?":
          return evaluateConditional(operands, context2);
        // Handle function calls
        case "()":
          return evaluateFunctionCall(operands, context2);
        // Handle property access
        case ".":
          return evaluatePropertyAccess(operands, context2);
        // Handle bracket notation access
        case "[]":
          return evaluateBracketAccess(operands, context2);
        // Handle sequence expressions (comma operator)
        case ",":
          return evaluateSequence(operands, context2);
        // Handle new operator
        case "new":
          return evaluateNew(operands, context2);
        // Handle class expressions
        case "class":
          return evaluateClass(operands);
        // Handle spread operator
        case "...":
          return evaluateSpread(operands, context2);
        // Handle function declarations
        case "function":
          return evaluateFunction(operands);
        // Handle async function declarations
        case "async function":
          return evaluateAsyncFunction(operands, context2);
        // Handle generator function declarations
        case "function*":
          return evaluateGeneratorFunction(operands, context2);
        // Handle arrow functions
        case "()=>":
          return evaluateArrowFunction(operands, context2);
        // Handle variable assignment
        case "=":
          return evaluateAssignment(operands, context2);
        case "+=":
          return evaluateAddAssignment(operands, context2);
        case "-=":
          return evaluateSubtractAssignment(operands, context2);
        case "*=":
          return evaluateMultiplyAssignment(operands, context2);
        case "/=":
          return evaluateDivideAssignment(operands, context2);
        case "%=":
          return evaluateModuloAssignment(operands, context2);
        case "<<=":
          return evaluateLeftShiftAssignment(operands, context2);
        case ">>=":
          return evaluateRightShiftAssignment(operands, context2);
        case ">>>=":
          return evaluateUnsignedRightShiftAssignment(operands, context2);
        case "&=":
          return evaluateBitwiseAndAssignment(operands, context2);
        case "^=":
          return evaluateBitwiseXorAssignment(operands, context2);
        case "|=":
          return evaluateBitwiseOrAssignment(operands, context2);
        // Handle await statement
        case "await":
          return evaluateAwait();
        // Handle line comment
        case "//":
          return evaluateLineComment();
        // Handle block comment
        case "/*":
          return evaluateBlockComment();
        // Handle return statement
        case "return":
          return evaluateReturn(operands, context2);
        // Handle yield statement
        case "yield":
          return evaluateYield();
        // Handle yield* statement
        case "yield*":
          return evaluateYieldStar();
        // Handle if statements
        case "if":
          return evaluateIf(operands, context2);
        // Handle while loops
        case "while":
          return evaluateWhile(operands, context2);
        // Handle for loops
        case "for":
          return evaluateFor(operands, context2);
        case "++":
          return evaluateIncrement(operands, context2);
        case "--":
          return evaluateDecrement(operands, context2);
      }
    }
        ...
        if (Array.isArray(body)) {
          for (let i = 0; i < body.length; i++) {
            ...
            if (item && typeof item === "object" && !Array.isArray(item)) {
              ...
              const functionDeclarationKey2 = itemKeys.find((key2) => key2.startsWith("function") && !key2.startsWith("function*"));
              if (functionDeclarationKey2) {
                let paramStr = "";
                let funcName = "";
                const nameMatch = functionDeclarationKey2.match(/function\s+(\w+)\s*\(([^)]*)\)/);
                if (nameMatch) {
                  funcName = nameMatch[1];
                  paramStr = nameMatch[2];
                }
                const funcParams = paramStr ? paramStr.split(",").map((p) => p.trim()).filter((p) => p) : [];
                const funcBody = item[functionDeclarationKey2];
                const functionResult = function(...funcArgs) {
                  const innerFunctionContext = {};
                  for (const key2 in functionContext) {
                    innerFunctionContext[key2] = functionContext[key2];
                  }
                  ...
                  if (Array.isArray(funcBody)) {
                    ...
                    for (const stmt of funcBody) {
                      result3 = evalJeon(stmt, innerFunctionContext);
                      if (stmt && typeof stmt === "object" && !Array.isArray(stmt) && stmt["return"] !== void 0) {
              ...
              const generatorFunctionDeclarationKey2 = itemKeys.find((key2) => key2.startsWith("function*"));
              if (generatorFunctionDeclarationKey2) {
                let paramStr = "";
                let funcName = "";
                const nameMatch = generatorFunctionDeclarationKey2.match(/function\*\s+(\w+)\s*\(([^)]*)\)/);
                if (nameMatch) {
                  funcName = nameMatch[1];
                  paramStr = nameMatch[2];
                }
                const funcParams = paramStr ? paramStr.split(",").map((p) => p.trim()).filter((p) => p) : [];
                const funcBody = item[generatorFunctionDeclarationKey2];
                const functionResult = function* (...funcArgs) {
                  const innerFunctionContext = {};
                  for (const key2 in functionContext) {
                    innerFunctionContext[key2] = functionContext[key2];
                  ...
                  if (Array.isArray(funcBody)) {
                    for (const stmt of funcBody) {
                      if (stmt && typeof stmt === "object" && !Array.isArray(stmt) && "yield" in stmt) {
                        ...
                        yield yieldValue;
                      } else if (stmt && typeof stmt === "object" && !Array.isArray(stmt) && stmt["return"] !== void 0) {
            ...
            const result2 = evalJeon(item, functionContext);
            if (item && typeof item === "object" && !Array.isArray(item) && item["return"] !== void 0) {
              return result2;
            }
            if (i === body.length - 1) {
    ...
    if (functionDeclarationKey) {
      ...
      if (funcName) {
        context2[funcName] = functionResult;
        return void 0;
      } else {
        return functionResult;
      }
    ...
    if (asyncFunctionDeclarationKey) {
      ...
      if (nameMatch) {
        ...
        context2[funcName] = async function(...args) {
          const functionContext = { ...context2 };
          params.forEach((param, index) => {
            functionContext[param] = args[index];
          });
    ...
    const generatorFunctionDeclarationKey = keys.find((key2) => key2.startsWith("function*"));
    if (generatorFunctionDeclarationKey) {
      ...
      if (nameMatch) {
        ...
        const paramStr = nameMatch[2];
        const params = paramStr ? paramStr.split(",").map((p) => p.trim()).filter((p) => p) : [];
        const body = jeon[generatorFunctionDeclarationKey];
        context2[funcName] = function* (...args) {
          const functionContext = { ...context2 };
          params.forEach((param, index) => {
            functionContext[param] = args[index];
          });
      ...
      if (typeof declarations === "object" && declarations !== null) {
        if ("=" in declarations) {
          ...
          if (Array.isArray(assignment) && assignment.length === 2) {
            ...
            const sourceValue = sourceExpr ? evalJeon(sourceExpr, context2) : {};
            if (Array.isArray(pattern)) {
              const destructuredVars = /* @__PURE__ */ new Set();
              for (let i = 0; i < pattern.length; i++) {
                ...
                if (typeof varName === "string" && varName.startsWith("...")) {
                  ...
                  if (Array.isArray(sourceValue)) {
                    ...
                    context2[restVarName] = value;
                  } else {
                    const restObj = {};
                    if (sourceValue && typeof sourceValue === "object") {
                      for (const key2 in sourceValue) {
                        if (!destructuredVars.has(key2)) {
                          restObj[key2] = sourceValue[key2];
                        }
                      }
                    }
                    context2[restVarName] = restObj;
                  }
                } else if (typeof varName === "string") {
                  if (varName.startsWith("[@") && varName.endsWith("]")) {
                    const inner = varName.substring(1, varName.length - 1);
                    const nestedVars = inner.split(",").map((v) => v.trim().substring(1));
                        ...
                        if (nestedSource && typeof nestedSource === "object" && !Array.isArray(nestedSource)) {
                          for (let j = 0; j < nestedVars.length; j++) {
                            ...
                            if (nestedVar && nestedVar in nestedSource) {
                              context2[nestedVar] = nestedSource[nestedVar];
                            } else {
                              context2[nestedVar] = void 0;
                            }
                          }
                          ...
                          for (let j = 0; j < nestedVars.length; j++) {
                            const nestedVar = nestedVars[j];
                            if (nestedVar) {
                              context2[nestedVar] = j < nestedSource.length ? nestedSource[j] : void 0;
                            }
                          }
                        } else {
                    ...
                    if (Array.isArray(sourceValue)) {
                      const value = i < sourceValue.length ? sourceValue[i] : void 0;
                      context2[varName] = value;
                    } else {
                      const value = sourceValue && typeof sourceValue === "object" && varName in sourceValue ? sourceValue[varName] : void 0;
                      context2[varName] = value;
                    }
                  } else {
                    destructuredVars.add(varName);
          ...
          for (const [varName, valueExpr] of Object.entries(declarations)) {
            const value = valueExpr === "@undefined" ? void 0 : evalJeon(valueExpr, context2);
            context2[varName] = value;
          }
        }
      }
...
function* createGeneratorFromStatements(statements, context2) {
  for (const stmt of statements) {
    if (stmt && typeof stmt === "object" && !Array.isArray(stmt) && "yield" in stmt) {
      const yieldValue = evalJeon(stmt["yield"], context2);
      yield yieldValue;
    } else if (stmt && typeof stmt === "object" && !Array.isArray(stmt) && "yield*" in stmt) {
      const iterable = evalJeon(stmt["yield*"], context2);
      ...
      const returnValue = evalJeon(stmt["return"], context2);
      return returnValue;
    } else {
          ...
          if (op === "while" && Array.isArray(operands) && operands.length === 2) {
            ...
            if (condition) {
              ...
              if (Array.isArray(operands[2])) {
                yield* createGeneratorFromStatements(operands[2], context2);
              } else {
                yield* createGeneratorFromStatements([operands[2]], context2);
              }
            }
          } else {
            evalJeon(stmt, context2);
          }
        } else {
          evalJeon(stmt, context2);
        }
...
var parse3 = function parse4(text, reviver) {
  ...
  key = void 0;
  root = void 0;
  Uint8ArrayHex = reviver && reviver.Uint8ArrayHex ? reviver.Uint8ArrayHex : globalThis.JSON5Options && globalThis.JSON5Options.Uint8ArrayHex ? globalThis.JSON5Options.Uint8ArrayHex : void 0;
  bigintSupport = reviver && reviver.bigint ? reviver.bigint : globalThis.JSON5Options && globalThis.JSON5Options.bigint ? globalThis.JSON5Options.bigint : void 0;
...
var stringify = function stringify2(value, replacer, space) {
  ...
  Uint8ArrayHex2 = replacer && replacer.Uint8ArrayHex || globalThis.JSON5Options && globalThis.JSON5Options.Uint8ArrayHex;
  bigintSupport2 = replacer && replacer.bigint || globalThis.JSON5Options && globalThis.JSON5Options.bigint;
  ...
  function serializeProperty(key2, holder) {
    ...
    if (typeof value2 === "object") {
      if (Uint8ArrayHex2 && value2 instanceof Uint8Array) {
        return value2.reduce(
...
return {
  ...
  const useJsxFunction = observable(false);
  ...
  const convertJeonToJs = () => {
    try {
      ...
      let code = jeon2js(jeon, { json: get(json), jsx: get(useJsxFunction) });
      if (get(useIIFE)) {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          const wrappedJeon = js2jeon(wrappedCode, { json: get(json) });
  ...
  const evaluateJsOutput = () => {
    try {
      ...
      const jeon = get(json).parse(jeonText);
      let context2 = {};
      try {
        const contextText = get(evalContext);
        context2 = contextText ? get(json).parse(contextText) : {};
      } catch (contextError) {
  ...
  return /* @__PURE__ */ jsx("div", { class: "max-w-6xl mx-auto p-5 bg-gray-800 text-white", children: /* @__PURE__ */ jsxs("div", { class: "bg-white rounded-lg shadow-lg p-6 my-6 text-gray-800", children: [
    ...
    /* @__PURE__ */ jsxs("div", { class: "flex items-center justify-left mb-4", children: [
      /* @__PURE__ */ jsx(
        "input",
        {
          type: "checkbox",
          id: "useJsxFunction",
          checked: get(useJsxFunction),
          onChange: (e) => useJsxFunction(e.target.checked),
          class: "mr-2"
        }
      ),
      /* @__PURE__ */ jsx("label", { for: "useJsxFunction", class: "text-gray-600", children: "Use JSX Function (produce jsx() calls instead of JSX syntax)" })
    ] }),
    ...
    /* @__PURE__ */ jsxs("div", { class: "flex flex-col lg:flex-row justify-center mb-4 gap-4", children: [
      /* @__PURE__ */ jsxs("div", { class: "w-full lg:w-1/2 relative", children: [
        ...
        /* @__PURE__ */ jsxs("div", { class: "mt-4 p-4 bg-green-50 rounded-lg border border-green-200", children: [
          /* @__PURE__ */ jsxs(
            "button",
            {
...
//# sourceMappingURL=index-ZoK-5NjX.js.map

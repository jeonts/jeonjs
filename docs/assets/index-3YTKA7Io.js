...
function visitJSX(keys, jeon, visit, jsonImpl) {
...
function visitClass(op, operands, keys, visit, jsonImpl) {
  if (op.startsWith("class ")) {
    ...
    const memberEntries = Object.entries(classMembers).map(([key2, value]) => {
...
function visitFunctionCall(keys, jeon, visit, jsonImpl) {
  const functionCallKey = keys.find((key2) => key2.endsWith("()"));
  if (functionCallKey) {
    const functionName = functionCallKey.substring(0, functionCallKey.length - 2);
    const functionArgs = jeon[functionCallKey];
    const args = Array.isArray(functionArgs) ? functionArgs.map((arg) => visit(arg)) : [];
...
function jeon2js(jeon, options2, isTopLevel = true) {
  ...
  if (Array.isArray(jeon)) {
    return visitorRegistry$1.visitArray(jeon, jeon2js, jsonImpl, isTopLevel);
  }
  if (typeof jeon === "object" && jeon !== null) {
    ...
    const functionDeclarationResult = visitorRegistry$1.visitFunctionDeclaration(keys, jeon, jeon2js, jsonImpl);
    ...
    if (keys.length === 1) {
      ...
      const operatorResult = visitorRegistry$1.visitOperator(op, operands, jeon2js, jsonImpl);
      if (operatorResult) {
        return operatorResult;
      }
      if (op === "try") {
        return visitorRegistry$1.visitTryCatch(operands, jeon2js, jsonImpl);
      }
      if (op === "@" || op === "@@") {
        return visitorRegistry$1.visitVariableDeclaration(op, operands, jeon2js, jsonImpl);
      }
      const arrowFunctionResult = visitorRegistry$1.visitArrowFunction(op, operands, jeon2js, jsonImpl);
      if (arrowFunctionResult) {
        return arrowFunctionResult;
      }
      const classResult = visitorRegistry$1.visitClass(op, operands, keys, jeon2js, jsonImpl);
      if (classResult) {
        return classResult;
      }
      const sequencingResult = visitorRegistry$1.visitSequencing(op, operands, jeon2js, jsonImpl);
      if (sequencingResult) {
        return sequencingResult;
      }
      const propertyAccessResult = visitorRegistry$1.visitPropertyAccess(op, operands, jeon2js, jsonImpl);
      if (propertyAccessResult) {
        return propertyAccessResult;
      }
      if (op === "switch") {
        return visitorRegistry$1.visitSwitch(operands, jeon2js, jsonImpl);
      }
    }
    const jsxResult = visitorRegistry$1.visitJSX(keys, jeon, jeon2js, jsonImpl);
    if (jsxResult) {
      return jsxResult;
    }
    const functionCallResult = visitorRegistry$1.visitFunctionCall(keys, jeon, jeon2js, jsonImpl);
    if (functionCallResult) {
      return functionCallResult;
    }
    return visitorRegistry$1.visitObject(keys, jeon, jeon2js, jsonImpl);
  }
...
function visitUnaryExpression(node, options2) {
  return {
    [node.operator]: ast2jeon(node.argument)
  };
}
function visitProgram(node, options2) {
  if (node.body.length === 1) {
    return ast2jeon(node.body[0]);
  }
  return node.body.map((stmt) => ast2jeon(stmt));
}
...
function visitConditionalExpression(node, options2) {
  return {
    "?": [
      ast2jeon(node.test),
      ast2jeon(node.consequent),
      ast2jeon(node.alternate)
    ]
  };
}
  ...
  if (node.callee.type === "MemberExpression") {
    ...
    return {
      "()": [
        {
          ".": [
            ast2jeon(callee.object),
            callee.property.type === "Identifier" ? callee.property.name : ""
          ]
        },
        ...node.arguments.map((arg) => ast2jeon(arg))
      ]
    };
  } else {
    const functionName = node.callee.name;
    return {
      [`${functionName}()`]: node.arguments.map((arg) => ast2jeon(arg))
    };
  }
}
function visitMemberExpression(node, options2) {
  ...
  while (current2 && current2.type === "MemberExpression") {
    if (current2.computed) {
      segments.unshift(ast2jeon(current2.property));
    } else {
      segments.unshift(
  ...
  segments.unshift(ast2jeon(current2));
  return {
    ".": segments
  };
}
function visitArrayExpression(node, options2) {
  return node.elements.filter((element) => element !== null).map((element) => ast2jeon(element));
}
function visitObjectExpression(node, options2) {
  ...
  for (const prop of node.properties) {
    if (prop.type === "Property") {
      ...
      obj[key2] = ast2jeon(prop.value);
    } else if (prop.type === "SpreadElement") {
      const spreadKey = spreadIndex === 0 ? "..." : `...${spreadIndex}`;
      obj[spreadKey] = ast2jeon(prop.argument);
      spreadIndex++;
    }
  }
  return obj;
...
function visitSwitchStatement(node, options2) {
  const cases = node.cases.map((caseClause) => {
    if (caseClause.test) {
      return {
        "case": ast2jeon(caseClause.test),
        "body": caseClause.consequent.map((consequent) => ast2jeon(consequent))
      };
    } else {
      return {
        "default": caseClause.consequent.map((consequent) => ast2jeon(consequent))
      };
    }
  });
  return {
    "switch": [
      ast2jeon(node.discriminant),
      cases
    ]
  };
}
...
function visitFunctionDeclaration(node, options2) {
  ...
  if (node.body.type === "BlockStatement") {
    ...
    return {
      [`${functionType}${functionName}${paramStr}`]: Array.isArray(blockBody.body) ? blockBody.body.map((stmt) => ast2jeon(stmt)) : [ast2jeon(blockBody.body[0])]
    };
  } else {
    return {
      [`${functionType}${functionName}${paramStr}`]: [ast2jeon(node.body)]
    };
  }
}
...
function visitArrowFunctionExpression(node, options2) {
  ...
  return {
    [`${prefix}${paramStr} =>`]: ast2jeon(node.body)
  };
}
...
function visitVariableDeclaration(node, options2) {
  ...
  for (const decl of node.declarations) {
    if (decl.id.type === "Identifier") {
      declarations[decl.id.name] = decl.init ? ast2jeon(decl.init) : null;
    }
  }
  return {
    [key2]: declarations
...
function visitIfStatement(node, options2) {
  ...
  if (node.alternate) {
    result["if"][2] = ast2jeon(node.alternate);
  }
  return result;
}
...
function visitForStatement(node, options2) {
  return {
    "for": [
      node.init ? ast2jeon(node.init) : null,
      node.test ? ast2jeon(node.test) : null,
      node.update ? ast2jeon(node.update) : null,
      ast2jeon(node.body)
    ]
  };
}
function visitReturnStatement(node, options2) {
  return {
    "return": node.argument ? ast2jeon(node.argument) : null
  };
}
    ...
    for (const member of node.body.body) {
      if (member.type === "MethodDefinition") {
        ...
        if (member.value.body.type === "BlockStatement") {
          ...
          const methodBody = Array.isArray(blockBody.body) ? blockBody.body.map((stmt) => ast2jeon(stmt)) : [ast2jeon(blockBody.body[0])];
          classMembers[methodKey] = methodBody;
        } else {
          classMembers[methodKey] = [ast2jeon(member.value.body)];
        }
...
function visitClassExpression(node) {
  node.id ? node.id.name : "AnonymousClass";
  const classMembers = {};
  if (node.body && node.body.body) {
    for (const member of node.body.body) {
      if (member.type === "MethodDefinition") {
        ...
        const methodBody = Array.isArray(member.value.body.body) ? member.value.body.body.map(ast2jeon) : [ast2jeon(member.value.body.body)];
        classMembers[methodKey] = methodBody;
      } else if (member.type === "PropertyDefinition") {
        let propName = "";
        if (member.key.type === "Identifier") {
...
function visitMethodDefinition(node) {
  ...
  const methodBody = Array.isArray(node.value.body.body) ? node.value.body.body.map(ast2jeon) : [ast2jeon(node.value.body.body)];
  const result = {
...
function visitYieldExpression(node, options2) {
  if (node.delegate) {
    return {
      "yield*": node.argument ? ast2jeon(node.argument) : null
    };
  } else {
    return {
      "yield": node.argument ? ast2jeon(node.argument) : null
...
function visitTryStatement(node, options2) {
  const tryResult = {
    "try": {
      "body": Array.isArray(node.block.body) ? node.block.body.map((stmt) => ast2jeon(stmt)) : [ast2jeon(node.block.body[0])]
    }
  };
  if (node.handler) {
    tryResult.try.catch = {
      "param": node.handler.param ? node.handler.param.name : "error",
      "body": Array.isArray(node.handler.body.body) ? node.handler.body.body.map((stmt) => ast2jeon(stmt)) : [ast2jeon(node.handler.body.body[0])]
    };
  }
  if (node.finalizer) {
    tryResult.try.finally = Array.isArray(node.finalizer.body) ? node.finalizer.body.map((stmt) => ast2jeon(stmt)) : [ast2jeon(node.finalizer.body[0])];
  }
  return tryResult;
}
const createVisitorWrapper = (visitor) => {
  return (node, options2) => {
    return visitor(node, options2);
  };
};
const visitorRegistry = {
  "BinaryExpression": createVisitorWrapper(visitBinaryExpression),
  "UnaryExpression": (node, options2) => visitUnaryExpression(node),
  "Program": (node, options2) => visitProgram(node),
  "Identifier": (node, options2) => visitIdentifier(node),
  "Literal": (node, options2) => visitLiteral(node),
  "LogicalExpression": (node, options2) => visitLogicalExpression(node),
  "ConditionalExpression": (node, options2) => visitConditionalExpression(node),
  "CallExpression": (node, options2) => visitCallExpression(node),
  "MemberExpression": (node, options2) => visitMemberExpression(node),
  "ArrayExpression": (node, options2) => visitArrayExpression(node),
  "ObjectExpression": (node, options2) => visitObjectExpression(node),
  "SpreadElement": (node, options2) => visitSpreadElement(node),
  "SwitchStatement": (node, options2) => visitSwitchStatement(node),
  "ExpressionStatement": (node, options2) => visitExpressionStatement(node),
  "NewExpression": (node, options2) => visitNewExpression(node),
  "FunctionDeclaration": (node, options2) => visitFunctionDeclaration(node),
  "FunctionExpression": (node, options2) => visitFunctionExpression(node),
  "ArrowFunctionExpression": (node, options2) => visitArrowFunctionExpression(node),
  "BlockStatement": (node, options2) => visitBlockStatement(node),
  "VariableDeclaration": (node, options2) => visitVariableDeclaration(node),
  "AssignmentExpression": (node, options2) => visitAssignmentExpression(node),
  "ArrayPattern": (node, options2) => visitArrayPattern(),
  "ObjectPattern": (node, options2) => visitObjectPattern(),
  "UpdateExpression": (node, options2) => visitUpdateExpression(node),
  "IfStatement": (node, options2) => visitIfStatement(node),
  "WhileStatement": (node, options2) => visitWhileStatement(node),
  "ForStatement": (node, options2) => visitForStatement(node),
  "ReturnStatement": (node, options2) => visitReturnStatement(node),
  "ClassDeclaration": (node, options2) => visitClassDeclaration(node),
  "ClassExpression": (node, options2) => visitClassExpression(node),
  "ThisExpression": (node, options2) => visitThisExpression(),
  "MethodDefinition": (node, options2) => visitMethodDefinition(node),
  "PropertyDefinition": (node, options2) => visitPropertyDefinition(node),
  "AwaitExpression": (node, options2) => visitAwaitExpression(node),
  "JSXElement": (node, options2) => visitJSXElement(node),
  "JSXText": (node, options2) => visitJSXText(node),
  "JSXExpressionContainer": (node, options2) => visitJSXExpressionContainer(node),
  "YieldExpression": (node, options2) => visitYieldExpression(node),
  "BreakStatement": (node, options2) => visitBreakStatement(node),
  "TryStatement": (node, options2) => visitTryStatement(node)
};
function ast2jeon(node, options2) {
  ...
  switch (node.type) {
    ...
    case "ObjectExpression":
      ...
      for (const prop of node.properties) {
        if (prop.type === "Property") {
          const key22 = prop.key.type === "Identifier" ? prop.key.name : prop.key.value;
          obj[key22] = ast2jeon(prop.value);
        } else if (prop.type === "SpreadElement") {
          const spreadKey = spreadIndex === 0 ? "..." : `...${spreadIndex}`;
          obj[spreadKey] = ast2jeon(prop.argument);
          spreadIndex++;
    ...
    case "ArrowFunctionExpression":
      ...
      if (node.type === "ArrowFunctionExpression") {
        ...
        return {
          [`${prefix}${paramStr} =>`]: ast2jeon(funcNode.body)
        };
      } else {
        ...
        if (funcNode.body.type === "BlockStatement") {
          ...
          return {
            [`${functionType}${functionName}${paramStr}`]: [ast2jeon(funcNode.body)]
          };
        }
      }
    case "BlockStatement":
    ...
    case "IfStatement":
      ...
      const result = {
        "if": [
          ast2jeon(ifStmt.test),
          ast2jeon(ifStmt.consequent)
        ]
      };
      if (ifStmt.alternate) {
        result["if"][2] = ast2jeon(ifStmt.alternate);
      }
      return result;
    case "WhileStatement":
      return {
    ...
    case "ForStatement":
      ...
      return {
        "for": [
          forStmt.init ? ast2jeon(forStmt.init) : null,
          forStmt.test ? ast2jeon(forStmt.test) : null,
          forStmt.update ? ast2jeon(forStmt.update) : null,
          ast2jeon(forStmt.body)
        ]
      };
    ...
    case "YieldExpression":
      ...
      if (yieldExpr.delegate) {
        return {
          "yield*": yieldExpr.argument ? ast2jeon(yieldExpr.argument) : null
        };
      } else {
        return {
          "yield": yieldExpr.argument ? ast2jeon(yieldExpr.argument) : null
    ...
    case "SwitchStatement":
      ...
      return {
        "switch": [
          ast2jeon(switchStmt.discriminant),
          cases
        ]
      };
    case "TryStatement":
      ...
      const tryResult = {
        "try": {
          "body": Array.isArray(tryStmt.block.body) ? tryStmt.block.body.map((stmt) => ast2jeon(stmt)) : [ast2jeon(tryStmt.block.body[0])]
        }
      };
      if (tryStmt.handler) {
        tryResult.try.catch = {
          "param": tryStmt.handler.param ? tryStmt.handler.param.name : "error",
          "body": Array.isArray(tryStmt.handler.body.body) ? tryStmt.handler.body.body.map((stmt) => ast2jeon(stmt)) : [ast2jeon(tryStmt.handler.body.body[0])]
        };
      }
      if (tryStmt.finalizer) {
        tryResult.try.finally = Array.isArray(tryStmt.finalizer.body) ? tryStmt.finalizer.body.map((stmt) => ast2jeon(stmt)) : [ast2jeon(tryStmt.finalizer.body[0])];
      }
      return tryResult;
    case "ThrowStatement":
      return {
        "throw": ast2jeon(node.argument)
      };
    case "Program":
      if (node.body.length === 1) {
        return ast2jeon(node.body[0]);
...
const App = () => {
  ...
  const tsInput = observable("function sum(a, b) {\n  return a + b;\n}");
  ...
  return /* @__PURE__ */ jsx$1("div", { class: "max-w-6xl mx-auto p-5 bg-gray-800 text-white", children: /* @__PURE__ */ jsxs("div", { class: "bg-white rounded-lg shadow-lg p-6 my-6 text-gray-800", children: [
    ...
    /* @__PURE__ */ jsxs("div", { class: "flex flex-col md:flex-row gap-6", children: [
      ...
      /* @__PURE__ */ jsxs("div", { class: "flex-1 flex flex-col", children: [
        /* @__PURE__ */ jsx$1("h2", { class: "text-xl font-semibold text-gray-800 mb-3", children: "JEON Output" }),
        /* @__PURE__ */ jsx$1(
          "pre",
          {
            id: "jeon-output",
    ...
    /* @__PURE__ */ jsxs("div", { class: "mt-10", children: [
      /* @__PURE__ */ jsx$1("h2", { class: "text-2xl font-bold text-gray-800 mb-5", children: "Example Conversions" }),
      ...
      /* @__PURE__ */ jsxs("div", { class: "mb-5 p-5 bg-gray-50 border-l-4 border-purple-500 rounded-r", children: [
        /* @__PURE__ */ jsx$1("h3", { class: "text-lg font-semibold text-purple-600 mb-3", children: "9. Assigned Class" }),
        /* @__PURE__ */ jsx$1("p", { class: "font-medium mb-2", children: "JEON:" }),
        /* @__PURE__ */ jsxs("div", { class: "collapsible-code", children: [
          /* @__PURE__ */ jsxs("button", { class: "collapsible-btn text-blue-600 hover:text-blue-800 font-medium flex items-center", children: [
            /* @__PURE__ */ jsx$1("span", { class: "mr-2", children: "▼" }),
      ...
      /* @__PURE__ */ jsxs("div", { class: "p-5 bg-gray-50 border-l-4 border-yellow-500 rounded-r", children: [
        /* @__PURE__ */ jsx$1("h3", { class: "text-lg font-semibold text-yellow-600 mb-3", children: "10. Object Spread Operator (JSON5)" }),
        /* @__PURE__ */ jsx$1("p", { class: "font-medium mb-2", children: "JEON:" }),
        /* @__PURE__ */ jsxs("div", { class: "collapsible-code", children: [
          /* @__PURE__ */ jsxs("button", { class: "collapsible-btn text-blue-600 hover:text-blue-800 font-medium flex items-center", children: [
            /* @__PURE__ */ jsx$1("span", { class: "mr-2", children: "▼" }),

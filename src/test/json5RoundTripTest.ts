...
test('JSON5 Round-trip Test', () => {
    test('Converts JS to JEON and back with JSON5 support', () => {
        ...
        const testKeys = ['function greet', 'special-key', 'another.key', 'unicode🔑', 'trailingComma']

        testKeys.forEach(key => {
            expect(normalizedRegenerated).toContain(key)
        })
        ...
        console.log('✅ Round-trip test PASSED - All key elements preserved')
